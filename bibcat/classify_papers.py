...
if (do_real_testdata):
    ...
    dict_TVTinfo = np.load(os.path.join(
        dir_info, "dict_TVTinfo.npy"), allow_pickle=True).item()
    list_test_bibcodes = [key for key in dict_TVTinfo if (
        dict_TVTinfo[key]["folder_TVT"] == folder_test)]
    ...
    list_test_indanddata_raw = [(ii, dataset[ii]) for ii in range(0, len(dataset))
                                if (dataset[ii]["bibcode"] in list_test_bibcodes)]
    ...
    for ii in range(0, len(list_test_indanddata)):
        ...
        # mission structure
        curr_info = {"text": curr_data["body"], "id": str(curr_ind),
            ...
            for curr_kobj in all_kobjs:
                ...
                # Otherwise, store that this mission was not detected for this text
                    ...
                    curr_info["missions"][curr_name] = {
                        "mission": curr_name,
                        "class": config.verdict_rejection}
...
operator_ML = classes.Operator(classifier=classifier_ML, mode=mode_modif,
                               keyword_objs=all_kobjs,
                               name="Operator_ML", do_verbose=True,
                               load_check_truematch=True,
                               do_verbose_deep=False)
# The rule-based operator
operator_RB = classes.Operator(classifier=classifier_rules,
                               name="Operator_RB", mode=mode_modif,
                               keyword_objs=all_kobjs,
...
# The rule-based operator
results_RB = operator_RB.classify(text=dict_texts_RB,
                                  lookup=lookup, buffer=buffer_RB,
                                  modif=mode_modif,
...
performer.evaluate_performance_basic(operators=list_operators,
                                     ...
                                     print_freq=1, do_verbose=True,
                                     do_verbose_deep=False, figsize=figsize)
...
performer.evaluate_performance_uncertainty(operators=list_operators,
                                           dicts_texts=list_dict_texts,
                                           mappers=list_mappers,
