...
def generate_directory_TVT(
    ...
    all_bibcodes_oneclassif = {
        key1: [key2 for key2 in unique_bibcodes if (dict_bibcode_classifs[key2] == [key1])] for key1 in unique_classes
    }
    ...
    # For mode where training sets should use fraction of data available
        ...
        dict_split = {
            key: (np.round((fraction_TVT * all_bibcodes_partitioned_counts[key]))).astype(int) for key in unique_classes
        }  # Partition per class per TVT

    # Otherwise, throw error if mode not recognized
    else:
        raise ValueError(
            ("Err: The given mode for generating the TVT" + " directory {0} is invalid. Valid modes are: {1}").format(
                mode_TVT, valid_modes
            )
        )
    ...
    if any([os.path.exists(os.path.join(dir_data, item)) for item in name_folderTVT]):
        raise ValueError(
            "Err: TVT directories exist in model directory"
            + " and will not be overwritten. Please remove or"
            + " change the given model directory (dir_data)."
            + "\nCurrent dir_data: {0}".format(dir_data)
        )
            ...
            for curr_bibcode in dict_bibcodes_perTVT[repr_key][ind_TVT]:
                ...
                for curr_textid in dict_bibcode_textids[curr_bibcode]:
                    ...
                    curr_filebase = os.path.join(
                        dir_data, name_folderTVT[ind_TVT], act_key
    ...
    for curr_key in unique_classes:
        ...
        curr_count = sum(
            [
                len(
                    [item2 for item2 in os.listdir(os.path.join(dir_data, item1, curr_key)) if (item2.endswith(".txt"))]
                )
