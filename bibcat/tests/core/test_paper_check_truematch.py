...
# -------------------------------
@pytest.mark.parametrize("phrase,lookup,expected", [
...
# -------------------------------
@pytest.mark.parametrize("phrase,lookup,expected", [
    ("The Hubble Space Telescope is a really neat telescope that is often referred to as Hubble or HST.", "Hubble", True),
    ("Once the transit is observable, we will observe the target in a timely fashion without delay and at high resolution using either JWST or Hubble.", "Hubble", None),
    ("Edwin Hubble's papers", "Hubble", None),
    ("high S/N Hubble image", "Hubble", None),
    ("HST observatory", "Hubble", None),
    ("H.S.T. observatory", "Hubble", None),
...
@pytest.mark.parametrize("phrase,lookup,expected", [
    ...
    ("Table 1 then gives the measured Kepler velocity data and Kepler planets.", "Kepler", ["measured Kepler velocity data", "Kepler planets"]),
])
...
@pytest.mark.parametrize("phrase,lookup,expected", [
    ...
    ("Table 1 then gives the measured Kepler velocity data and Kepler planets.", "Kepler", [
        ("measured Kepler velocity data", False, "Kepler velocity"),
        ("Kepler planets", False, "Kepler planet"),
    ]),
    ("The Kepler images and Kepler plots indicate a correlation.", "Kepler", [
...
def test_consider_wordchunk(paper_setup, phrase, lookup, expected):
    ...
    list_wordchunks = testpaper._assemble_keyword_wordchunks_wrapper(setup_data)
    list_results = [
        testpaper._consider_wordchunk(chunk, setup_data) for chunk in list_wordchunks
    ]
    test_res = [
...
# -------------------------------
@pytest.mark.parametrize("phrase,lookup,expected", [
    ("Hubble's calibrated data", "Hubble", [("Hubble's calibrated data", "hubble calibrated data datum", ["hubble"])]),
    ("Edwin Hubble's papers", "Hubble", [("Edwin Hubble's papers", "edwin hubble composition document newspaper paper", ["hubble"])]),
    ("Hubble et al. 2000", "Hubble", [("Hubble et al", "hubble et alabama aluminum", ["hubble"])]),
    ("Kepler velocity fields.", "Kepler", [("Kepler velocity fields", "kepler speed airfield battlefield discipline field fields plain playing_field sphere", ["kepler"])]),
    ("We summarize our Hubble results next.", "Hubble", [("our Hubble results", "our hubble consequence result resultant_role solution", ["hubble"])]),
])
def test_setup_consider_wordchunk(paper_setup, phrase, lookup, expected):
    ...
    list_wordchunks = testpaper._assemble_keyword_wordchunks_wrapper(setup_data)
    list_results = [
        testpaper._setup_consider_wordchunk(chunk, setup_data) for chunk in list_wordchunks
    ]
    test_res = [
        (repr(chunk), result[0], result[1]) for chunk, result in zip(list_wordchunks, list_results)
    ]
    assert test_res == expected
...
# -------------------------------
@pytest.mark.parametrize("phrase,lookup,expected", [
    ("Edwin Hubble's papers", "Hubble", {"Edwin Hubble's papers": ["matcher", "bool", "text_wordchunk", "text_database"]}),
    ("A Hubble constant data", "Hubble", {"Hubble constant data": ["matcher", "bool", "text_wordchunk", "text_database"]}),
    ("Hubble et al. 2000", "Hubble", {"Hubble et al": ["matcher", "bool", "text_wordchunk", "text_database"]}),
    ("Kepler velocity fields.", "Kepler", {"Kepler velocity fields": ["matcher", "bool", "text_wordchunk", "text_database"]}),
    ("Table 1 then gives the measured Kepler velocity data and Kepler planets.", "Kepler", {
